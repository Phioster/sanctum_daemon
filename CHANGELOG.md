...
- **Music plays again, and says so when it cannot.** The audit's session callback
  admitted only this package — but media3 routes the lock screen, Bluetooth and car
  head units through the platform session under a fixed sentinel package name, so the
  callback shut them out. It is withdrawn; the credential it was guarding left the
  metadata in the same round, which was always the real fix. Underneath it sat a worse
  problem: three layers of the music path each swallowed their own error, so a failure
  produced no message, no log line and no clue. All three speak now.
